...
//        printBoard(board);
    ...
    public static void printHome(ArrayList<int[]> input){
    ...
    public static void calculatePay(int coverage){
        // i, j is standard location
        int cost=1;
        ...
        for(int i = 0; i<board.length; i++){
            for(int j = 0; j<board[0].length; j++){
                ...
                for(int k = 0; k<home.size(); k++){
                    int dist = Math.abs(i - home.get(k)[0]) + Math.abs(j-home.get(k)[1]);
                ...
                if(temp_result >= 0 && count > result){
                    result = count;
//                    System.out.println("Update Result : " + i + " " + j + " " + coverage);
